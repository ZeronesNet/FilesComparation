...
from ErrorOperation import PathfileExistError
...
class Hash_Process(object):
    ...
    def get_path(self):
        ...
        if all_file_list == []:
            error = PathfileExistError("2","File Not Exist")
            raise error
        elif path.exists(self.file_path) and all_file_list == []:
            file_list_1.append(self.file_path)
        ...
        for i in range(len(file_path) - 1,-1,-1):  # 逆序读取列表
            ...
            with open(single_file_path,"rb") as f1:
                ...
                if length == None:
                    ...
                    print("\033[32m[Success]\033[0m 计算完成\t[{: <5}\\{: >5}]\t{}".format(str(len(file_path)),list_length,single_file_path,outputnum = len(list_length)))
